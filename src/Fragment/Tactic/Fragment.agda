...
mutual
  record Operator : Set where
    ...
    normalised : MacroCtx → TC Term
    normalised ctx =
      normalise (n-ary arity (apply raw (vra (vec (debrujin arity)) ∷ [])))
...
ctx : Name → Term → Term → TC MacroCtx
ctx frex model goal
  ...
  where modelErr : ∀ {a} {A : Set a} → Term → String → TC A
        ...
        extract-interp : CoreCtx → Name → Term
        extract-interp ctx op = def (quote _⟦_⟧_) ( vra (CoreCtx.theory ctx)
                                                  ∷ vra (CoreCtx.model ctx)
                                                  ∷ vra (con op [])
                                                  ∷ []
                                                  )
        ...
        extract-ops ctx
          = do ops ← normalise (def (quote ops) (vra (CoreCtx.signature ctx) ∷ []))
               ...
               let interps = map (extract-interp ctx) symbols
