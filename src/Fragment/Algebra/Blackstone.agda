...
module Fragment.Algebra.Blackstone (Σ : Signature) where
...
module _
  ...
  bind-args≡map : ∀ {n} {xs : Vec (Term ∥ A ∥) n}
                  → Pointwise (_≈ₜ_ ∥ B ∥/≈) (bind-args ∥ f ∥ₛ xs)
                                             (map (bind ∥ f ∥ₛ) xs)
  bind-args≡map {xs = []}     = []
...
joinₕ : ∀ {A : Algebra {a} {ℓ₁}} → Free (Herbrand ∥ A ∥/≈) →ₕ Free ∥ A ∥/≈
joinₕ {A = A} = bindₕ {A = Free ∥ A ∥/≈} {B = A} idₛ

substₕ : ∀ {A : Algebra {a} {ℓ₁}} {B : Algebra {b} {ℓ₂}}
         → (∥ A ∥/≈ →ₛ ∥ B ∥/≈) → Free ∥ A ∥/≈ →ₕ B
substₕ {A = A} {B = B} θ =
  (evalₕ B) ∘ₕ bindₕ {A = A} {B = B} (atomₛ ∥ B ∥/≈ ∘ₛ θ)
